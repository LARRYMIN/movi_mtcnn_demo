...
public:
	...
	vector<FaceInfo> Detect(const cv::Mat& img, const int min_size, const float* threshold, const float factor, const int stage, int a, int b, int c, int d);
protected:
	...
	void GenerateBBox(std::shared_ptr<float> confidence, std::shared_ptr<float> reg_box,  int feature_map_h_, int feature_map_w_, float scale, float thresh);
